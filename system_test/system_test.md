...
| Vehicle stopped | same topic, `stopped: true` on the closest in-range vehicle | — | VEHICLE_STOP → CHECK_VEHICLE_STOPPED immediately, then → CHECK_PLATE once `stopped: true` has held continuously for `vehicle_stopped_confirm_s` (default 1.0s) — debounces a single noisy reading, same idea as `vehicle_lost_confirm_s` for the vehicle leaving. Keep publishing (`-r 10`) for at least that long, not just a burst |
...
| Close proximity (base scan) | `/perception/close_proximity` | `std_msgs/Bool` | MIDDLE_IDLE only (suppressed in any other state): plays `pedestrian_alert_message` + `pedestrian_alert_expression` (angry face) instead of the introduction clip/face — same range/motion behavior as above, distinguished from a classified pedestrian purely by which topic triggered it. **Takes priority** over a simultaneous pedestrian detection. Face reverts to `idle_expression` (neutral) once neither signal is active |
...
| Robot stuck in `CHECK_VEHICLE_IN_RANGE`, never `RETURNING` | `crossing_zone_monitor_node` isn't launched, or `zone_x`/`zone_y` in its config are still the placeholder zeros | `_crossing_zone_clear` fails safe — a stale/missing `/perception/crossing_zone_occupied` reading is treated as NOT clear, on purpose. Launch `crossing_zone_monitor` and calibrate its zone corners (same `tf2_echo` approach as `pose_a`/`pose_b`) |
